...
@retry
def get_creation_year(session: Client, user_id: int) -> int:
    response = session.get(f"https://users.roblox.com/v1/users/{user_id}")
    if response.status_code != 200:
        raise ValueError(f"Failed to fetch creation year (status {response.status_code})")
    created = response.json().get("created")

    if not created:
        raise ValueError("Missing created date")
    ...
    try:
        ...
        try:
            created_at = datetime.strptime(normalized, "%Y-%m-%dT%H:%M:%S.%f%z")
        except ValueError:
            created_at = datetime.strptime(normalized, "%Y-%m-%dT%H:%M:%S%z")
...
class AccountInfo:
    ...
    @staticmethod
    def get_creation_year(session: Client, user_id: int):
        return get_creation_year(session, user_id)